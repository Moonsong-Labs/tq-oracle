...
from ...clients import EtherscanLogsClient
...
class StakeWiseAdapter(BaseAssetAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        self._etherscan_client = self._build_etherscan_client(
            config,
            stakewise_defaults,
    ...
    ) -> EtherscanLogsClient | None:
        """Build an Etherscan logs client if API key is configured."""
        api_key = api_key_override or getattr(defaults, "etherscan_api_key", None)
        if not api_key:
            return None

        page_size = page_size_override or getattr(defaults, "etherscan_page_size", 1000)
        return EtherscanLogsClient(
            api_key=api_key,
    ...
    async def _scan_exit_queue_tickets(
        ...
        event_names = [e.event_name for e in context.exit_events]
        logger.debug(
            "Scanning exit queue — vault=%s user=%s events=%s (checking owner+receiver filters)",
            context.address,
            user,
            event_names,
        )
        ...
        logger.debug(
            "Exit queue scan complete — vault=%s user=%s unique_tickets=%d (from %d logs)",
            context.address,
            user,
            len(ordered),
            len(seen_logs),
